...
var oaController = {
    sortPage: function (tbID, field, sortControl) {
        ...
        //设置升序、降序图标
        var arImg = ["/Content/images/arrow-up.png", "/Content/images/arrow-down.png"];
        var imgSort = "";
...
$(function () {
    ...
    loadMenus();
    setframelayout();
    $("#oa-split").css('background', "url('/Content/images/leftarrow.png') no-repeat center center");
...
function initPopMenu() {
    var html = [];
    html.push(" <ul id=\"menu-pop-ul\">");
    html.push("        <li></li>");
    html.push("        <div class=\"menu-pop-ul-content\">");
    html.push("            <ul id=\"menu-pop-ul-content\">");
    html.push("");
    html.push("            </ul>");
    html.push("        </div>");
    html.push("    </ul>");
...
function setlayout() {
    ...
    var middleRow2 = [{ id: "oa-list", width: "fill", height: "fill" }];
    var pageHeight = 35;
    var ctlPageHeight = $('#oa-page').attr('data-height');
    if (ctlPageHeight != undefined && ctlPageHeight != null && ctlPageHeight != '') {
        pageHeight = ctlPageHeight;
    }
    var bottomRow3 = [{ id: "oa-page", width: "fill", height: pageHeight }];
    $.layoutMY(topRow1, middleRow2, bottomRow3, $("#oa-right"), 0, 0, 3, 2);
}
...
function bindLeftEvent() {
    $("#oa-split").click(function () {
        ...
        if (leftW <= 0) {
            setframelayout();
            setlayout();
            $(this).css('background', "url('/Content/images/leftarrow.png') no-repeat center center");
            $("#oa-left").css('overflow-y', 'auto');
        }
        else {
            setlayoutHidLeft();
            $(this).css('background', "url('/Content/images/rightarrow.png') no-repeat center center");
...
function menuMouseHover() {
    $("#oa-app ul li").mouseenter(function () {
        var $obj = $(this);
        var point = $obj.offset();
        $("#menu-pop-ul").show().css({ top: point.top, left: point.left + 50 });
        $("#menu-pop-ul li").eq(0).html($(this).attr("title"));
        var app = $(this).find("a").eq(0).attr("name");
        ...
        $("#menu-pop-ul-content").html(html.join(" "));

    }).mouseout(function () {

    }).hover(function () {
        $("#oa-app ul li").removeClass("hover");
        $(this).addClass("hover");

    });

    $("#menu-pop-ul").hover(function () {
...
function timeStamp2String(time) {
    ...
    if (/T/ig.test(time)) {
        time = time.substring(0, time.indexOf('.')).replace(/-/ig, "/");
        time = time.replace("T", " ");
...
function openPostWindow(url, data) {
    ...
    for (var key in data) {
        ...
        if (typeof (val) === "object") {
            ...
            tempForm.appendChild(select);
        }
        else {
            var hideInput = document.createElement("input");
            hideInput.type = "hidden";
            hideInput.name = key; //后台要接受这个参数来取值
            hideInput.value = val; //后台实际取到的值
